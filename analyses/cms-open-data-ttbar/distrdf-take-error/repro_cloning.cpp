...
ROOT::RVecF jet_pt_resolution(std::size_t size)
{
   ...
   ROOT::RVecF res(size);
   std::generate(std::begin(res), std::end(res), []() { return get_thread_local_trandom().Gaus(1, 0.05); });
   return res;
}
...
std::tuple<ROOT::RDF::RResultPtr<TH1D>, ROOT::RDF::Experimental::RResultMap<TH1D>> analysis(ROOT::RDF::RNode &df)
{
   ...
   // Default weights
   auto d1 = df.Define("weights", [&xsec_weight]() { return xsec_weight; });

   auto pt_scale_up = 1.03;
   ...
   auto d2 = d1.Vary("jet_pt",
                     [&pt_scale_up](const ROOT::RVecF &jet_pt) {
                        return ROOT::RVec<ROOT::RVecF>{jet_pt * pt_scale_up, jet_pt * jet_pt_resolution(jet_pt.size())};
                     },
                     {"jet_pt"}, {"pt_scale_up", "pt_res_up"});
   ...
   auto d3 =
      d2.Define("jet_pt_mask", [](const ROOT::RVecF &jet_pt) { return jet_pt > 25; }, {"jet_pt"})
   ...
   auto d4 = d3.Vary("weights",
                     [](double weights, const ROOT::RVecF &jet_pt, const ROOT::RVecI &jet_pt_mask) {
   ...
   auto d5 =
      d4.Filter([](const ROOT::RVecD &jet_btag,
                   const ROOT::RVecI &jet_pt_mask) { return ROOT::VecOps::Sum(jet_btag[jet_pt_mask] >= 0.5) == 1; },
                {"jet_btag", "jet_pt_mask"})
         .Define(observable,
